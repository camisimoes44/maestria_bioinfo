            ...
            [
                ...
                dbc.FormGroup(
                    [
                        ...
                        # user name, formtext
                        dbc.Label('User name:', id='profile-user'),
                        dbc.FormText('Cannot change user name', color='secondary'),
                        html.Br(),
                        ...
                        # level, formtext
                        dbc.Label('Level:', id='profile-level'),
                        dbc.FormText('Cannot change level', color='secondary'),
                        html.Br(),
...
@app.callback(
    ...
    [Output('profile-' + x, 'children') for x in ['first', 'last', 'email', 'user', 'level']] + \
...
def profile_values(trigger):
    ...
    """
    if not trigger:
        return no_update, no_update, 'First: ', 'Last: ', 'Email:', 'User name:', 'Level:', '', ''
    if current_user.is_authenticated:
        return (
            ...
            ['User name: ', html.Strong(current_user.user)],
            ['Level: ', html.Strong(current_user.level)],
            current_user.first,
            current_user.last
        )
    return login_alert, '/login', 'First: ', 'Last: ', 'Email:', 'User name:', 'Level:', '', ''
...
def profile_validate(first, last, password, confirm):
    ...
    if sum([1 if (v is False and inv is False) or (v is True and inv is False) else 0 for v, inv in
